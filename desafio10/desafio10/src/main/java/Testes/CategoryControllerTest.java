...
public class CategoryControllerTest {
    ...
    public void testandoMetodoCreate() throws Exception {
        Category category = new Category(5L, "Entrada1", "ItemDeTeste", Types.Conta);
    ...
    public void testandoMetodoUpdate() throws Exception {
        ...
        mockMvc.perform(put("/categorias", category.getId()))
                .andExpect(status().isOk())
                        .andExpect(content().contentType(MediaType.APPLICATION_JSON));
        verify(categoryService, times(1)).update(category, 5L);
        verifyNoMoreInteractions(categoryService);
    }
